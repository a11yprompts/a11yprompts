...
findings:
  - id: F-005
    sc: 1.1.1
    component: Team photo — Alex Rivera
    note: Placeholder SVG treated as decorative but not hidden from screen readers; alt text prefixed with "Photo of"
  - id: F-006
    sc: 1.1.1
    component: Team photo — Jordan Lee
    note: Placeholder SVG treated as decorative but not hidden from screen readers; alt text prefixed with "Photo of"
  - id: F-007
    sc: 1.1.1
    component: Team photo — Sam Okafor
    note: Placeholder SVG treated as decorative but not hidden from screen readers; alt text prefixed with "Photo of"
observed_failures:
  - tool: claude-code
...
<!--
  confidence: medium
  Pattern-based rules (alt presence, avoiding "photo of" prefixes) are reliably applied
  by AI. The informative vs. decorative judgment call is context-dependent — AI cannot
  determine whether adjacent text makes an image decorative without understanding the
  full page context. Verify generated output manually.
-->
...
<!-- Alt text presence is axe-detectable. Alt text quality and the informative vs.
     decorative judgment require manual verification. -->
...
Detects missing `alt` attributes. Cannot evaluate whether alt text is meaningful, or
whether an image should be decorative based on surrounding context.

**Axe Pro — Intelligent Guided Tests**
F-005, F-006, F-007 were detected via Axe Pro guided testing — the guided test surfaces
SVG elements exposed to AT for human confirmation.

**Manual**
Navigate images using screen reader image shortcuts (NVDA: `G`, VoiceOver: `VO+Cmd+G`).
Confirm decorative images are skipped and informative images announce meaningful text.
